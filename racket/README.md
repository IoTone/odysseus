...
the app depends on them.
    ...
    racket/
      ...
      test/run-tests.rkt            portable rackunit suite (18 cases)
...
> build/run/test.
    ...
    racket test/run-tests.rkt                 # the suite (expect: 18 success(es))
...
DB-backed tools (`manage_notes`, `manage_tasks`) operate on the app database
...
Fastest signal → fullest:
    ...
    racket test/run-tests.rkt                                # 2. behavior (18 tests)
