...
$verNum = ($ver | Select-String -Pattern '\d+\.\d+\.\d+').Matches.Value
if ($verNum) {
  $p = $verNum.Split('.'); $maj=[int]$p[0]; $min=[int]$p[1]
  if ($maj -eq 0 -and $min -lt 3) { Die "ollama $verNum is too old for tool_calls — need >= 0.3.0; update from ollama.com" }
}
